...
first_layer = layers.Conv2D(512, (7, 7), strides=(3,3), padding='same', activation='relu', name='first_conv_7x7')(input_placeholder)
...
for throwaway in range(0,10):
  ...
  for prune_target in prune_targets:
    ...
    while (True):
      ...
      prune_outputs = identify_by_gradient.get_prune_by_gradient(model, layer, datagen, prune_intensity = 0.4)

      print(prune_outputs)

      # if there are only a few outputs to pruen, lets move on to the next one. 
