...
for full_pass_id in range(1, num_of_full_passes+1):
  ...
  for prune_target in prune_targets:
    ...
    while (True):
      ...
      prune_outputs = identify_by_gradient.get_prune_by_gradient(model, layer, datagen, prune_intensity = (float(full_pass_id) / float(num_of_full_passes)))

      print("Pruning out these layers:", prune_outputs)
