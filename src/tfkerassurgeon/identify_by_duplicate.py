...
class DuplicateActivationIdentifier:
    """
    This class uses the similarities of activations/outputs to identify which layer outputs are good candidates for pruning, 
    and return a list of votes, that score each Output based on prune priority

    Arguments:
        batch_size: the size of each batch returned by the generator
        check_for_inverse_corr (Optional): applied to the output, it is a power (allowing better distinction between items that are 95% zeros and 75% zeros)
    ...
    """
    def __init__(self, batch_size, check_for_inverse_corr = False):
        #  Set the static values
        self.batch_size = batch_size
        self.check_for_inverse_corr = check_for_inverse_corr
    ...
    def get_votes(self, model, layer, data_generator, num_of_batches = 10):
        ...
        return self.get_duplicate_scores(model, layer, data_generator, self.batch_size, num_of_batches, self.check_for_inverse_corr)


    def get_batch_dup_scores (self, activations, non_channel_axis, check_for_inverse_corr, channels_count):
    ...
    def get_duplicate_scores(self, model, layer, data_generator, batch_size, num_of_batches, check_for_inverse_corr):
        """
        ...
        for i in range(0, num_of_batches):
            ...
            scores = self.get_batch_dup_scores (activations, non_channel_axis, check_for_inverse_corr, channels_count)
